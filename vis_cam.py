...
def parse_args():
    ...
    parser.add_argument('--img', default="/home/zys/images/test" ,help='Image file')
    parser.add_argument('--config',default="configs/Mynet/my.py", help='train config file path')
    parser.add_argument('--checkpoint',default="work_dirs/my/epoch_1.pth", help='checkpoint file')
    parser.add_argument(
        '--target-layers',
        default=['vision_encoder.vision_encoder.base_model.model.layers.23'],
        nargs='+',
        type=str,
    ...
    parser.add_argument(
        '--save-path',
        default="/home/zys/wnmd/fuck/images/fuck/res2/",
        type=Path,
        help='The path to save visualize cam image, default not to save.')
    parser.add_argument('--device', default='cuda', help='Device to use cpu')
    parser.add_argument(
        '--vit-like',
        default="true",
        action='store_true',
...
def reshape_transform(tensor, model, args):
    ...
    if tensor.ndim == 4:
        ...
        # For (B, L, C)
        num_extra_tokens =  tensor.size()[1]-args.num_extra_tokens 
...
def main():
    ...
    data_listA=os.listdir(args.img+"/A/")
    data_listB=os.listdir(args.img+"/B/")
    i=0
    for (imgA,imgB) in zip(data_listA,data_listB):
        imgAA=args.img+"/A/"+imgA
        imgBB=args.img+"/B/"+imgB
        img_path=[imgAA,imgBB]
        ...
        # 123123123123126835184615824 in myminigpt4 to change outputs to avoid "list has no cpu"
        grayscale_cam = cam(
            data,
            targets,
            eigen_smooth=args.eigen_smooth,
            aug_smooth=args.aug_smooth)
        out_path=str(args.save_path)+'/'+imgA
        i+=1
        print("now is "+str(i)+out_path+imgB+"all is"+str(len(data_listA))+'\r',end='')
        show_cam_grad(
